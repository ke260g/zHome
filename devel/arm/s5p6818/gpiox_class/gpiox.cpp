...
/* This is the gpio-manipulation for s5p6818 */
 ...
 * @GPIOA is defined in `enum gpiox_ops_group'
 */

static volatile unsigned int *GPIOxBASE[5];

enum GPIOxREGS_OFFSET {
    /* refer to s5p6818-manual-16.6 */
    GPIOxOUT = 0,       /*  0, 0x00 */
    GPIOxOUTENB,        /*  1, 0x04 */
...
static int __gpiox_ops(const int group,
    ...
    if(pin_num > 15)
        ...
        *(GPIOxBASE[group] + GPIOxALTFN0) &= ~(1 << (pin_num*2));

    /* special pin */ /* 01'b for gpio-function */
    if((group == GPIOC) &&
            (pin_num == 13 ||
             pin_num == 14 ||
             pin_num == 8  ||
             pin_num == 7  ||
             pin_num == 4)) {
        *(GPIOxBASE[group] + GPIOxALTFN0) &= ~(1 << (pin_num*2 + 1));
        *(GPIOxBASE[group] + GPIOxALTFN0) |= (1 << (pin_num * 2));
    } else if(((group == GPIOC) && (pin_num == 24)) || // pin_num == 28)) ||
        ((group == GPIOB) &&
         (pin_num == 26 || pin_num == 28 || pin_num == 29 || pin_num == 30 || pin_num == 31))) {
             printf("Beautiful.\n");
             *(GPIOxBASE[group] + GPIOxALTFN1) &= ~(1 << ((pin_num-16)*2+1));
             *(GPIOxBASE[group] + GPIOxALTFN1) |= (1 << ((pin_num-16)*2));
         }
#endif
