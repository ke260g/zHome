...
*/
    ...
    if(desc.iManufacturer >= 0) {
        ...
        libusb_get_string_descriptor_ascii(
            handle, desc.iManufacturer, description, 1024);
...
// #elif 0
    if(desc.iProduct >= 0) {
        memset(description, 0, 1024);
        libusb_get_string_descriptor_ascii(
            handle, desc.iProduct, description, 1024);
        printf("%s ", description);
    }
...
#endif
#if 1
    ret = libusb_get_port_numbers(dev, port_arr, PORT_LENGTH);
    printf("\n\t>>>Port List: ");
    for(i = 0; i < ret; i++)
        printf("#%d ", port_arr[i]);
    printf("end");
    ...
    if(config_desc->iConfiguration >= 0) {
        memset(description, 0, 1024);
        libusb_get_string_descriptor_ascii(
                handle, config_desc->iConfiguration, description, 1024);
        printf("%s ", description);
    }
